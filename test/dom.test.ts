...
import { browse, createDocument, parseHTML } from '../src'
...
describe('Virtual Page', () => {
  ...
  test('should find elements', async () => {
    await browse(async (page) => {
      await page.goto('https://example.com')
      const element = await page.querySelector('body')
  ...
  test('should handle localStorage', async () => {
    await browse(async (page) => {
      ...
      await page.removeLocalStorage('test')
      const removed = await page.getLocalStorage('test')
      expect(removed).toBeNull()
    })
  })

  test('should handle sessionStorage', async () => {
    await browse(async (page) => {
      await page.setSessionStorage('test', 'value')
      const value = await page.getSessionStorage('test')
      expect(value).toBe('value')

      await page.clearSessionStorage()
      const cleared = await page.getSessionStorage('test')
  ...
  test('should handle cookies', async () => {
    await browse(async (page) => {
      ...
      const deleted = await page.getCookie('test')
  ...
  test('should handle assertions', async () => {
    await browse(async (page) => {
      await page.goto('https://example.com')
      await page.assertSee('Example')
      await page.assertDontSee('NonExistent')
    })
  })
})
